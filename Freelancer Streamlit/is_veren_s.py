...
def isi_kaydet(baslik, aciklama,butce,tarih,hz_is,spotifi_linko,sarki):
    try:
        conn = None
        cursor = None
        ...
        query = "INSERT INTO isler (proje_basligi,proje_aciklamasi,proje_butce,proje_tarihi,proje_sahibi,proje_verildi_mi,hizli_is_mi,spotifi_link,begenilecek_sarki) VALUES (?,?,?,?,?,?,?,?,?)"
        cursor.execute(query, (baslik, aciklama,butce,tarih.strftime("%Y-%m-%d"),ana_kullanici_ad,False,hz_is,spotifi_linko,sarki))
        conn.commit()
...
def is_tanimla():
    ...
    hiz_check = st.checkbox("Hızlı iş/Normal İş")
    if hiz_check:
        hizli_is_mi=True
        sarki_isim = st.text_input("Kullanıcıya beğendirmek istediğiniz şarkının ismi: ")
        spotifi_linki = st.text_input("Spotify' da beğendirmek istediğiniz şarkının linkini giriniz:")
        st.write("Uyarı!!! Şarkı linki (spotify:track:27NovPIUIRrOZoCHxABJwK) formatında olmalıdır.")
    else:
        hizli_is_mi=False
        ...
        if proje_basligi and proje_aciklama and proje_butce and proje_zaman:
            if not kayit_var_mi(proje_basligi):
                if not proje_basligi.isdigit() and not proje_aciklama.isdigit():
                    isi_kaydet(proje_basligi, proje_aciklama,proje_butce ,proje_zaman,hizli_is_mi,spotifi_linki,sarki_isim)
                    time.sleep(1)
...
def atanmis_isler():
    ...
    if selected_value:
        ...
        if not df.empty:
            ...
            basvuru_id = df[df['proje_basligi'] == selected_value]['proje_ID'].values[0]
            hizli_mi = df[df['proje_basligi'] == selected_value]['hizli_is_mi'].values[0]
